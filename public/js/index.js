...
const showImages = (images) => {
  ...
  images.forEach((image) => {
    ...
    imagesLocal.innerHTML += `
            ...
            <img src="http://localhost:4460/api/${image.id}/show" class="figure-img img-fluid rounded" alt="...">
    ...
    imagesRemota.innerHTML += `
              <figure id="${image.id}-rm" class="figure col-3">
                ...
                <figcaption class="figure-caption text-end">Imagen Guardada en Cloudinary</figcaption>
              </figure>
                `;
  });
};
...
const deleteImage = async (id) => {
  const response = await fetch(`http://localhost:4460/api/${id}/destroy`, {
    method: "DELETE",
  });
